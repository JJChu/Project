...
function appleMenu(ev){
	...
	aItem.each(function(index,item){
		d=getDistance(item);
		d=Math.min(d, iMax);
		$(item).css('width',((iMax-d)/iMax)*90+50+"px");
	})
}
//单击出现弹框
...
// 切换appList
var nowAppList = 'appList1';
$('.top_nav').on('click','.menu_item',function(){
	$('.appList').animate({'left':document.documentElement.clientWidth},300);
	$('.'+this.dataset.url).animate({'left':0},300);
	nowAppList = this.dataset.url;
})
...
var appList = $('.appList');
appList.css('left',document.documentElement.clientWidth);
$('.appList1').css('left',0);
//生成app HTML
...
//生成桌面应用
appList.each(function(index){
	//app列表
	$(this).html(appHtml(fileData['appList'+(index+1)]));
	//app位置
	appPos(appList[index]);
})

function appPos(appList){
...
$(window).on('resize',function(){
	...
	$('.appList').not($('.'+nowAppList)[0]).css('left',document.documentElement.clientWidth);
});	
...
function upFn(){
	var appItems = $('.'+nowAppList+' .appItem');
		...
		if (appItems[i] === target.get(0)) {
			...
			if (hit(target.get(0),appItems[i])) {	//碰撞到
				...
				if (hitTarget.dataset.id == 1) {
					fileData.appList1[0].deleData.push( dataFn.findDataById(target[0].dataset.id,fileData.appList1)[0] );
					dataFn.deleDataById(target[0].dataset.id,fileData.appList1);	//删除数据
					target.remove();
				} else{
					hitLeft = appItems[i].offsetLeft;
					hitTop = appItems[i].offsetTop;
					hitTargetId = hitTarget.dataset.id;
					dataFn.exchangeData(targetId,hitTargetId,fileData[nowAppList]);	//交换数据
				}
				break;
			}else{
				hitTarget = null;
...
function deleFile(data){	// 回收站文件夹
	var html = '<ul class="filesList">';
	for (var i = 0; i < data.length; i++) {
		html += `<li class="appItem" diaonoff=${data[i].diaOnoff} data-id=${data[i].id} data-name=${data[i].name} data-url=${data[i].url}>
					<img src=${data[i].ico} />
					<span>${data[i].name}</span>
				</li>`
	}
...
$(document).on('click',function(ev){
	...
	switch(target){
		...
		case $('.menu_list>li')[3]:				//新建文件夹
			...
			console.log(nowAppList);
			$('.'+nowAppList).html( appHtml(fileData[nowAppList]) );
			console.log(fileData[nowAppList]);
			appPos($('.'+nowAppList)[0]);
			break;
